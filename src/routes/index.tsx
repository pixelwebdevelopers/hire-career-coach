...
function Home() {
  return (
    <>
      ...
      {/* Testimonials */}
      <section className="relative">
        <div className="mx-auto max-w-7xl px-5 sm:px-8 py-24 sm:py-32 border-t border-border">
          <Reveal>
          ...
          <div className="mt-16 grid gap-6 lg:grid-cols-3">
            {TESTIMONIALS.map((t, i) => (
              <Reveal key={t.name} delay={i * 120} className="rounded-3xl bg-card border border-border p-8 flex flex-col">
                <div className="flex gap-1 text-gold">
                  {Array.from({ length: 5 }).map((_, k) => <Star key={k} className="h-4 w-4 fill-current" />)}
                </div>
                <p className="font-display italic text-2xl leading-snug mt-6 flex-1">"{t.quote}"</p>
